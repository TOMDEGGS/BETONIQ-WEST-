...
async function getBriefing() {
  ...
  try {
    ...
    if (res.ok) {
      let full = await res.text();
      // v7.3 fix — the full vault is ~14KB (~3600 tokens), which alone blows most of Groq's
      // free-tier per-minute token budget on every single message, causing constant rate-limit
      // failures. Cap what gets injected into the system prompt; use search_database /
      // get_database_summary tools for anything beyond this core summary.
      if (full.length > 2200) {
        full = full.slice(0, 2200) + "\n\n[...briefing truncated for token budget — use search_database or get_database_summary tools for full detail...]";
      }
      cachedBriefing = full;
...
MANDATORY RULE — NUMBERS/COUNTS: The knowledge base below is a trimmed summary — it does NOT contain live counts, totals, or full record lists. ANY question involving "how many", counts, totals, lists of records, or specific figures about projects/investors/merchants/agents/transactions/leads/subscriptions MUST trigger a call to get_database_summary (for broad "how many/what do we have" questions) or search_database (for specific lookups) BEFORE you answer. Never answer a numbers/count question from the knowledge base text alone — if you're not sure a number is live-verified, call the tool first. Only skip the tool call for pure personality/strategy/opinion questions that have nothing to do with database records.
...
async function callGroqRaw(model, apiKey, messages, tools, toolChoice) {
  const res = await fetch("https://api.groq.com/openai/v1/chat/completions", {
    ...
    body: JSON.stringify({ model, messages, temperature: 0.7, tools, tool_choice: toolChoice || "auto" }),
  });
  if (!res.ok) {
...
const NUMBERS_KEYWORD_RE = /\b(how many|count|total|totals|number of|list of|list all|summary|breakdown|how much)\b/i;

async function askGroqWithTools(systemPrompt, history, userMessage, senderId) {
  const apiKey = process.env.GROQ_API_KEY;
  if (!apiKey) return null;

  const forceToolFirstCall = NUMBERS_KEYWORD_RE.test(userMessage);
  ...
  for (const model of GROQ_MODELS) {
    try {
      ...
      while (loopCount < 4) {
        loopCount++;
        // Force a tool call on the very first turn for numbers/count-style questions —
        // "auto" tool_choice is sometimes skipped by the model even with strong prompt wording.
        const toolChoice = (loopCount === 1 && forceToolFirstCall) ? "required" : "auto";
        const result = await callGroqRaw(model, apiKey, currentMessages, TOOLS, toolChoice);
        if (!result.ok) {
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // DIAG777 — temporary diagnostic intercept, now captures the REAL askAI() error trail.
    if (/^diag777$/i.test(body)) {
      const g = process.env.GROQ_API_KEY || "";
      const gem = process.env.GEMINI_API_KEY || "";
      const info = [
        `GROQ len=${g.length}`,
        `GEMINI len=${gem.length}`,
      ].join(" | ");

      const captured = [];
      const origError = console.error;
      console.error = (...args) => {
        captured.push(args.map(a => (a && a.message) ? a.message : String(a)).join(" "));
        origError(...args);
      };

      let realReply = "not run";
      try {
        const briefingTxt = await getBriefing();
        const sysPrompt = SYSTEM_PERSONA + "\n\n" + briefingTxt;
        realReply = await askAI(sysPrompt, [], "How many real estate projects do we have right now?", "diag-sender");
      } catch (e) {
        realReply = "TOP-LEVEL EXCEPTION: " + e.message + " | stack=" + (e.stack || "").slice(0, 300);
      } finally {
        console.error = origError;
      }

      const errLog = captured.length ? captured.join(" || ").slice(0, 900) : "no errors captured";

      return {
        statusCode: 200,
        headers: { "Content-Type": "text/xml" },
        body: twimlReply(`${info} :: REPLY=${String(realReply).slice(0,300)} :: ERRORS=${errLog}`),
      };
    }
